...
def _evaluate(fg: int, vix, micro: str, vix_risk: str) -> tuple[Signal, float, str]:
    ...
    if fg <= 10:
        score += 2.5
        reasons.append(f"Extreme Fear ({fg}) — deep contrarian buy")
    elif fg <= 25:
        score += 2.0
        ...
        score -= 2.5
        reasons.append(f"Extreme Greed ({fg}) — deep contrarian sell")
    elif fg >= 75:
        score -= 2.0
        ...
        reasons.append(f"Neutral fear/greed ({fg})")
    # VIX dampens everything
    if vix_risk == "EXTREME":
        score *= 0.0
        reasons.append("VIX extreme — all signals muted")
    elif vix_risk == "ELEVATED":
        score *= 0.5
        reasons.append("VIX elevated — signals halved")
    # Micro impulse confirmation
    if micro == "BULLISH":
        score += 0.5
        reasons.append("Micro impulse bullish")
    elif micro == "BEARISH":
        score -= 0.5
        reasons.append("Micro impulse bearish")
